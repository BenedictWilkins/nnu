...
# -*- coding: utf-8 -*-
"""
    Created on 18-03-2021 10:53:41

    Allows async loading of a directory of HDF5 files via a torch DataLoader.
    ...
    Original Author: Branislav Holländer
"""
...
class HDF5Dataset(data.Dataset):
    ...
    def __init__(self, file_path, recursive, load_data, data_cache_size=3, transform=None):
        ...
        assert(p.is_dir())
        if recursive:
            files = sorted(p.glob('**/*.h5') + p.glob('**/*.hdf5'))
        else:
            files = sorted(p.glob('*.h5')  + p.glob('*.hdf5'))
        if len(files) < 1:
